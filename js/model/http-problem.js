...
HttpProblem.fromHttpError = function (httpError, detail) {
  ...
  var url = 'https://github.com/RHeactor/nucleus/wiki/HttpProblem#' +
    httpError.status +
...
HttpProblem.fromException = function (exception, status) {
  if (exception.name === 'HttpProblem') {
    return exception
  }
  var url = 'https://github.com/RHeactor/nucleus/wiki/HttpProblem#' +
    status +
  ...
  return new HttpProblem(url, exception.name, status, exception.toString())
}
